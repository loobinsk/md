...
class CalculationEnums(APIView):
	...
	def get(self, request, pk):
		...
		sales = SalesInitSerializer(sales, many=True, fields=('id', 'variant_name'))
		opexs = OpexVariant.objects.filter(project=project)
		opexs = OpexVariantSerializer(opexs, many=True, fields=('id', 'variant_name'))
		capexs = Capex.objects.filter(project=project)
		capexs = CapexSerializer(capexs, many=True, fields=('id', 'variant_name'))
		taxs = TaxPrm.objects.filter(project=project)
		taxs = TaxPrmSerializer(taxs, many=True, fields=('id', 'variant_name'))
		discount_rates = DiscountRate.objects.filter(project=project)
		discount_rates = DiscountRateSerializer(discount_rates, many=True, fields=('id', 'variant_name'))
		own_funds = OwnFundVariant.objects.filter(project=project)
		own_funds = OwnFundVariantSerializer(own_funds, many=True, fields=('id', 'variant_name'))
		credits = CreditVariant.objects.filter(project=project)
		credits = CreditVariantSerializer(credits, many=True,fields=('id', 'variant_name'))
		leasings = LeasingContractVariant.objects.filter(project=project)
		leasings = LeasingContractVariantSerializer(leasings, many=True, fields=('id', 'variant_name'))
		wk = WorkingCapitalParameter.objects.filter(project=project)
		wk = WorkingCapitalParameterSerializer(wk, many=True, fields=('id', 'variant_name'))
		data = {
			'sales': sales.data,
			'opexs': opexs.data,
			'capexs': capexs.data,
